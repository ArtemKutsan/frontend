...
const loadPost = async (id) => {
  try {
    const response = await fetch(
      `${BASE_URL}/posts/${postId}?_expand=user&_embed=comments`
    );
...
const commentCard = (comment) => {
  return `
    <div class="comment mb-4 p-4 bg-lite text-sm">
      <span class="comment-name font-semibold">${capitalizeFirstLetter(
        comment.name
      )}</span>
...
const renderPost = (post) => {
  ...
  postDiv.innerHTML = `
    ...
    <span class="post-author text-xs text-lite mb-2">Комментарии (${
